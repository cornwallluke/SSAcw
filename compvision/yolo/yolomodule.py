...
classesFile = os.path.join(os.path.dirname(__file__), 'coco.names')#args.class_file
classes = None
...
net = cv2.dnn.readNetFromDarknet(os.path.join(os.path.dirname(__file__), 'yolov3.cfg'),os.path.join(os.path.dirname(__file__), 'yolov3.weights'))#args.config_file, args.weights_file)
...
net.setPreferableTarget(cv2.dnn.DNN_TARGET_CPU)#OPENCL)
...
def getName(id):
    return classes[id]
def objectsInFrame(frame, conf):
    
    tensor = cv2.dnn.blobFromImage(cv2.resize(frame,(416,416)), 1/255, (inpWidth, inpHeight), [0,0,0], 1, crop=False)
    ...
    # remove the bounding boxes with low confidence
    confThreshold = conf#cv2.getTrackbarPos(trackbarName,windowName)
    classIDs, confidences, boxes = postprocess(frame, results, confThreshold, nmsThreshold)
